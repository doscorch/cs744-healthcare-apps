 ...
 */
router.post('/updatePassword', isAuthenticated, function (req, res, next) {
 ...
 */
router.post('/updateSecurityQuestions', isAuthenticated, function (req, res, next) {
...
router.post('/answerquestion', function (req, res, next) {
    _userService.answerQuestion(req.body, function (err, correct) {
        if (correct) {
            ...
            req.session.user.is_validated = true;
        } else {
            req.session.user.answer_attempt += 1;
        }
